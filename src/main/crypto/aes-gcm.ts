...
export function encryptAesGcm(plaintext: Buffer): EncryptResult {
  ...
  // Prepend nonce + append auth tag (matches nMobile Go GCM convention)
...
export function decryptAesGcm(
  ...
  // Split auth tag from end
  const encrypted = ciphertextWithTag.subarray(
    0,
    ciphertextWithTag.length - AUTH_TAG_BYTES,
  );
