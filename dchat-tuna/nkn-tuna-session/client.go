...
// SetCachedPubAddrs pre-populates the pubAddr cache for a remote address.
// This allows DialWithConfig to skip getPubAddrsFromRemote (NKN messaging)
// and directly use the provided relay addresses.
func (c *TunaSessionClient) SetCachedPubAddrs(remoteAddr string, pubAddrs *PubAddrs) {
	c.Lock()
	defer c.Unlock()
	c.pubAddrs[remoteAddr] = pubAddrs
	if c.addrConnCount[remoteAddr] == nil {
		c.addrConnCount[remoteAddr] = make(map[string]int)
	}
	for _, addr := range pubAddrs.Addrs {
		if addr != nil && addr.IP != "" && addr.Port != 0 {
			key := fmt.Sprintf("%s:%d", addr.IP, addr.Port)
			if c.addrConnCount[remoteAddr][key] <= 0 {
				c.addrConnCount[remoteAddr][key] = 1
			}
		}
	}
}
