...
- **Contact avatars** — Contact avatar photos displayed everywhere: message bubbles (inbound sender), session list (direct chats), chat header, topic subscriber panel, private group member panel, invite contact picker, user profile panel. Falls back to letter-initial circles when no avatar available. Uses `dchat-media://contact-cache/${avatarUri}` protocol. (`src/renderer/components/chat/MessageBubble.tsx`, `SessionList.tsx`, `MessageThread.tsx`, `PrivateGroupMemberPanel.tsx`)
- **Rich text rendering** — Automatic detection and rendering of markdown and HTML in message content, matching nMobile's behavior (which uses `flutter_markdown` as fallback). HTML sanitized via DOMPurify, markdown rendered via react-markdown. Styled headings, blockquotes, code blocks, lists, tables, links. External links open in system browser. Plain text fallback for messages without markdown/HTML. (`src/renderer/components/chat/MessageBubble.tsx`, `src/renderer/styles/global.css`)
...
| Rich Text | react-markdown + DOMPurify (markdown/HTML rendering) | Installed |
